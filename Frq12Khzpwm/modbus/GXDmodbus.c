...
/* ----------------------- Static variables ---------------------------------*/
static volatile  eMBEventType    eMBPollEvent;;
static volatile eMBRcvState eRcvState;
volatile unsigned char  ucRTUBuf[MB_SER_PDU_SIZE_MAX];
volatile unsigned char usRcvBufferPos;


unsigned char MBSlaveAddr;//从地址
...
void MBRTUSend(unsigned char *pucFrame, unsigned char usLength )
{
    ...
    usCRC16 = usMBCRC16(pucFrame, usLength );
    pucFrame[usLength++] = ( UCHAR )( usCRC16 & 0xFF );
    pucFrame[usLength++] = ( UCHAR )( usCRC16 >> 8 );
    for(i=0; i<usLength; i++)
    {
        if( eRcvState == STATE_RX_IDLE )
        {

            MBRTUSendChar(pucFrame[i]);
        }
        else
        {
            return;
...
void IRQ_xMBRTUTimerT35Expired( void )
{
    ...
    case STATE_RX_RCV:
        eMBPollEvent=EV_FRAME_RECEIVED;
        break;
